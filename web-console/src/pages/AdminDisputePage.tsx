...
export default function AdminDisputePage() {
  ...
  const listQ = useQuery({
    ...
    queryFn: async () => (await api.get<{ data: { list: Dispute[] } }>("/admin/disputes", { params: { status: filter || undefined } })).data.data.list,
    retry: 0,
  });
  const disputes = listQ.data ?? [];

  const resolveMut = useMutation<any, unknown, { action: "refund" | "dismiss"; resolution: string; refund_amount: number }>({
    mutationFn: async ({ action, resolution, refund_amount }) =>
      (await api.post(`/admin/disputes/${handling!.id}/resolve`, { action, resolution, refund_amount })).data,
    onSuccess: (d: { data?: { message?: string } }) => {
      toast.success(d?.data?.message ?? "处理成功");
  ...
  function handleDispute(action: "refund" | "dismiss") {
    if (!handling) return;
    resolveMut.mutate({ action, resolution, refund_amount: action === "refund" ? handling.amount : 0 });
  }
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={() => setHandling(null)}>
          <div style={{ background: "#fff", borderRadius: 10, padding: 24, maxWidth: 520, width: "90%" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 12 }}>
              ...
              <button onClick={() => handleDispute("dismiss")} disabled={resolveMut.isPending || !resolution.trim()} style={{ padding: "8px 16px", background: "#666", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", opacity: !resolution.trim() ? 0.6 : 1 }}>驳回争议 <HelpIcon text="驳回后争议关闭，用户余额不变化。" /></button>
              <button onClick={() => handleDispute("refund")} disabled={resolveMut.isPending || !resolution.trim()} style={{ padding: "8px 16px", background: "#22c55e", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", opacity: !resolution.trim() ? 0.6 : 1 }}>退款 ¥{(handling.amount / 100).toFixed(2)} <HelpIcon text="按争议金额退款至用户余额，并写入资金流水。" /></button>
            </div>
          </div>
        </div>
      )}
