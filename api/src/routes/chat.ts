...
import { estimateMultimodalContentTokens } from '../services/billing/multimodal-counter.js';
      ...
      const result = await runPipeline(pipelineCtx, [
        ...
        settleStep({
          implement: async (c) => {
            ...
            // 幂等：user-scoped 缓存键 + 请求指纹（写缓存的键须与 replay 查询键一致，
            // 指纹用于回放前异摘要拦截；见 services/idempotency.ts buildRequestFingerprint）
