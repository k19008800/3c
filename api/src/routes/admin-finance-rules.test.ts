...
describe('GET /admin/finance/rules（B17 读配置）', () => {
  it('admin 读取 → 200 且含 large_amount/limits/operation_2fa 默认值', async () => {
    ...
    expect(data.manual_topup.max_amount).toBe(1000000);
...
describe('PUT /admin/finance/rules（B17 保存配置）', () => {
  it('admin 改 large_amount/limits → 200 + resetFinanceRulesCache 即时生效（创建端点按新阈值定级）', async () => {
    ...
    // 即时生效：single_review_max=20000 → 创建 15000 人工上账 → tier1（旧阈值 10000 会定 tier2）
    const create = await app.inject({
      method: 'POST', url: '/api/v1/admin/manual-topup', headers: adminOp,
      payload: { user_id: activeUserId, amount: 15000, note: '新阈值定级' },
    });
    expect(create.statusCode).toBe(201);
    expect(create.json().data.approval_level).toBe(1);   // 15000 ≤ 新 single_review_max 20000
  });
