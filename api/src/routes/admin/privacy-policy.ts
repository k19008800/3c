// ============================================================
//  3cloud (3C) — 隐私政策版本管理路由（管理员）
//  GET    /api/v1/admin/settings/privacy-policy/versions         — 版本列表（分页）
//  POST   /api/v1/admin/settings/privacy-policy/versions         — 创建版本（草稿）
//  PUT    /api/v1/admin/settings/privacy-policy/versions/:id     — 编辑版本
//  POST   /api/v1/admin/settings/privacy-policy/versions/:id/publish — 发布版本
//  GET    /api/v1/admin/settings/privacy-policy/stats            — 统计信息
// ============================================================

import { FastifyInstance } from "fastify";
import { eq, desc, sql, and } from "drizzle-orm";
import { getDb } from "../../db/index.js";
import { privacyPolicyVersions, userPrivacyConsents, users, auditLogs } from "../../db/schema.js";
import { authenticateJWT, requirePerm, Perm } from "../../middleware/auth.js";

export async function adminPrivacyPolicyRoutes(app: FastifyInstance) {
  app.addHook("preHandler", authenticateJWT);

  // ── 版本列表（分页）──
  app.get("/api/v1/admin/settings/privacy-policy/versions", {
    ...
    const conditions = [];
    if (status) {
      conditions.push(eq(privacyPolicyVersions.status, status));
    }
    ...
    const rows = await db
      .select({
        ...
        status: privacyPolicyVersions.status,
        publishedAt: privacyPolicyVersions.publishedAt,
        createdBy: users.nickname,
        createdAt: privacyPolicyVersions.createdAt,
        updatedAt: privacyPolicyVersions.updatedAt,
      })
      .from(privacyPolicyVersions)
      .leftJoin(users, eq(privacyPolicyVersions.createdBy, users.id))
      .where(whereClause)
      .orderBy(desc(privacyPolicyVersions.createdAt))
      .limit(pageSize)
      .offset(offset);
  ...
  // ── 创建版本（草稿）──
  app.post("/api/v1/admin/settings/privacy-policy/versions", {
    ...
    const [maxVersionResult] = await db
      .select({ maxVersion: sql<string | null>`max(${privacyPolicyVersions.version})` })
      .from(privacyPolicyVersions)
    ...
    await db.insert(auditLogs).values({
      ...
      ip: request.ip,
      description: `创建隐私政策版本: v${created.version}${created.title ? ` (${created.title})` : ""}`,
    });
  ...
  // ── 编辑版本 ──
  app.put("/api/v1/admin/settings/privacy-policy/versions/:id", {
    preHandler: [requirePerm(Perm.CONFIG_EDIT)],
  }, async (request, reply) => {
    const db = getDb();
    ...
    await db.insert(auditLogs).values({
      ...
      after: updates,
      ip: request.ip,
      description: `更新隐私政策版本 v${before.version}: ${Object.keys(updates).join(", ")}`,
    });
  ...
  // ── 发布版本 ──
  app.post("/api/v1/admin/settings/privacy-policy/versions/:id/publish", {
    preHandler: [requirePerm(Perm.CONFIG_EDIT)],
  }, async (request, reply) => {
    const db = getDb();
    const id = parseInt((request.params as any).id, 10);
    ...
    await db.insert(auditLogs).values({
      ...
      ip: request.ip,
      description: `发布隐私政策版本 v${before.version}${before.title ? ` (${before.title})` : ""}`,
    });
  ...
  // ── 统计信息 ──
  app.get("/api/v1/admin/settings/privacy-policy/stats", {
    preHandler: [requirePerm(Perm.CONFIG_VIEW)],
    ...
    if (currentVersion) {
      const [currentConsentedResult] = await db
        .select({ count: sql<number>`count(*)` })
        .from(userPrivacyConsents)
        .where(eq(userPrivacyConsents.versionId, currentVersion.id));
