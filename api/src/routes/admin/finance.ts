...
export async function adminFinanceRoutes(app: FastifyInstance) {
  ...
  app.post("/api/v1/admin/finance/rates", {
    preHandler: app.rbac.preHandler(["FINANCE_ACTION"]),
    schema: {
      body: {
        type: "object",
  ...
  app.get("/api/v1/admin/finance/rates/history", {
    ...
    const query = request.query as any;
    const where = query.currency
      ? eq(exchangeRateHistory.currency, query.currency)
      : undefined;
