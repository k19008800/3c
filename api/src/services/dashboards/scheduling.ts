// ============================================================
//  3cloud (3C) — Dashboard Scheduling Realtime Service
//  GET /api/v1/admin/dashboard/scheduling-realtime — 模型调度
//  数据源：Redis 实时计数器（零 PostgreSQL 开销）
// ============================================================

import type { Redis } from "ioredis";

export interface SchedulingRealtimeResult {
  code: number;
  data: {
    minutes: number;
    series: Array<{
      time: string; rpm: number; tpm: number; avgLatencyMs: number;
    ...
    currentDistribution: Array<{
      vendorName: string; rpm: number; percentage: number; avgLatencyMs: number;
      topModels: Array<{ modelName: string; rpm: number; tpm: number }>;
    }>;
    lastUpdated: string;
    summary: {
      totalRpm: number; totalTpm: number; avgLatencyMs: number;
      peakRpm: number; peakTpm: number; avgLatencyRecent: number;
      vendorCount: number; modelCount: number;
    };
  };
  message: string;
}

export async function buildSchedulingRealtime(_db: any, redis: Redis, minutes: number): Promise<SchedulingRealtimeResult> {
  const now = new Date();
  const series: Array<{
    time: string; rpm: number; tpm: number; avgLatencyMs: number;
    models: Array<{ modelName: string; rpm: number; tpm: number }>;
  ...
  for (let i = minutes - 1; i >= 0; i--) {
    ...
    const bucket = `${t.getFullYear()}${String(t.getMonth() + 1).padStart(2, "0")}${String(t.getDate()).padStart(2, "0")}${String(t.getHours()).padStart(2, "0")}${String(t.getMinutes()).padStart(2, "0")}`;
    ...
    series.push({
      time, rpm: totalRpm, tpm: totalTpm, avgLatencyMs,
      models: Array.from(modelMap.entries()).map(([name, d]) => ({ modelName: name, rpm: d.rpm, tpm: d.tpm })),
      vendors: Array.from(vendorMap.entries()).map(([name, d]) => ({ vendorName: name, rpm: d.rpm, tpm: d.tpm })),
    });
  }
  ...
  const currentDistribution = latest
    ? Array.from(
        series[series.length - 1].vendors.reduce((acc, v) => {
          ...
          if (existing) { existing.rpm += v.rpm; } else { acc.set(v.vendorName, { vendorName: v.vendorName, rpm: v.rpm }); }
          return acc;
        }, new Map<string, { vendorName: string; rpm: number }>())
      ).map(([vendorName, v]) => ({
        vendorName, rpm: v.rpm,
        percentage: latest.rpm > 0 ? Math.round((v.rpm / latest.rpm) * 100) : 0,
        avgLatencyMs: latest.avgLatencyMs,
        topModels: series[series.length - 1].models.filter((m) => m.rpm > 0).sort((a, b) => b.rpm - a.rpm).slice(0, 3),
      }))
    : [];
  ...
  return {
    code: 0,
    data: {
      minutes, series, currentDistribution, lastUpdated: new Date().toISOString(),
      summary: {
        totalRpm: latestEntry?.rpm ?? 0, totalTpm: latestEntry?.tpm ?? 0,
        ...
        vendorCount: allVendors.size, modelCount: allModels.size,
      },
    },
    message: "ok",
  };
