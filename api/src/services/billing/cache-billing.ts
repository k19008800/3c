...
/**
 * 缓存命中折扣率 — 命中 token 按全价的 10% 计费。
 *
 * DeepSeek 官方即按 10% 收取缓存命中费用；OpenAI 官方为 50%，当前统一按保守的 10%
 * （对平台更有利、对用户更优惠）。后续如需按供应商差异化，可在 pricing 中扩展配置。
 */
 ...
 * - 有缓存命中：
 *   hit × inputPrice × CACHE_HIT_DISCOUNT + (input - hit) × inputPrice + output × outputPrice
 *
...
export function computeCacheDiscountedCost(
  ...
  const discountedCost =
    (hit / 1000) * pricing.input * CACHE_HIT_DISCOUNT +
 ...
 *
 * 规则：
 * - usage 存在且有缓存命中字段 → 命中部分按 CACHE_HIT_DISCOUNT 打折
...
export function parseAndDiscount(
  ...
  return computeCacheDiscountedCost(inputTokens, outputTokens, pricing, parseCacheTokens(usage));
}
