...
BEGIN
    ...
    FROM pg_class WHERE relname = 'commission_logs';
    ...
    IF is_partitioned THEN
        ...
        FOR partition_name IN
            ...
            FROM pg_class
            WHERE relkind = 'r'
            AND relname LIKE 'commission_logs_%'
        LOOP
            EXECUTE format('CREATE INDEX IF NOT EXISTS comm_logs_client_call_idx ON %I (client_call_log_id) WHERE client_call_log_id IS NOT NULL', partition_name);
        END LOOP;
        RAISE NOTICE 'Created indexes on commission_logs partitions';
    ELSE
        -- 非分区表，直接创建
...
BEGIN
    ...
    IF is_partitioned THEN
        FOR partition_name IN
            ...
            FROM pg_class
            WHERE relkind = 'r'
            AND relname LIKE 'call_logs_%'
        LOOP
            EXECUTE format('CREATE INDEX IF NOT EXISTS call_logs_key_item_idx ON %I (key_group_item_id, price_source) WHERE key_group_item_id IS NOT NULL', partition_name);
        END LOOP;
        RAISE NOTICE 'Created indexes on call_logs partitions';
    ELSE
