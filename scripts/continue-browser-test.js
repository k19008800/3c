...
// 读取测试计划
const testPlanPath = path.join(__dirname, '../BROWSER-FUNCTIONAL-TEST-PLAN.md');
const testReportPath = path.join(__dirname, '../BROWSER-TEST-REPORT.md');

// 测试进度
const testProgress = {
  // 模块1：总览看板 (6个页面)
  module1: {
    completed: ['管理仪表盘', '企业数据分析', '聚合统计'],
    pending: ['熔断看板', '系统健康', '趋势洞察'],
    currentIndex: 3 // 下一个要测试的是第4个
  },
  
  // 模块2：用户运营 (5个页面)
  module2: {
    completed: [],
    pending: ['用户管理', '实名审核', '额度管理', '管理 API Key', '角色权限'],
    currentIndex: 0
  },
  ...
  module3: {
    completed: [],
    pending: ['模型管理', '供应商管理', 'Key 分组', '模型映射', '供应商自助', '代理商管理'],
    currentIndex: 0
  },
...
// 生成继续测试的指南
function generateContinuationGuide() {
  const guide = `# 3cloud 浏览器测试继续指南

## 当前测试进度
- ✅ 已测试页面: ${testProgress.testedPages}/49
- 🔄 待测试页面: ${testProgress.remainingPages}
- 📊 完成度: ${((testProgress.testedPages / testProgress.totalPages) * 100).toFixed(1)}%
...
\`\`\`bash
# 更新测试进度
echo "测试进度: ${testProgress.testedPages}/49" >> test-report.md
...
// 保存继续指南
const continuationGuide = generateContinuationGuide();
fs.writeFileSync(
  path.join(__dirname, '../CONTINUE-TESTING-GUIDE.md'),
  continuationGuide
);

console.log('继续测试指南已生成: CONTINUE-TESTING-GUIDE.md');
console.log('\n下一步建议:');
console.log('1. 测试熔断看板页面');
console.log('2. 测试系统健康页面');
console.log('3. 测试趋势洞察页面');
console.log('4. 更新测试报告');

// 导出测试进度
module.exports = { testProgress, generateContinuationGuide };