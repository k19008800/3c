...
> **P0 补充**：2026-07-30 — 核心配置变更的审批流程、变更影响评估、灰度发布、回滚方案

### 9.1 配置变更分级

| 等级 | 描述 | 示例 | 审批人 | 生效方式 |
|------|------|------|--------|---------|
| L1-安全 | 影响安全策略的配置 | 登录锁定阈值、IP 白名单、密码策略 | super_admin | 即时 + 审计 |
| L2-财务 | 影响资金/定价的配置 | 全局加价率、信用额度、提现冻结期 | finance_admin 审批 | 定时生效 |
| L3-运营 | 影响运营策略的配置 | 限流阈值、活动参数、佣金率 | ops_admin 审批 | 即时或定时 |
| L4-常规 | 不影响核心业务的配置 | 站点名称、LOGO、联系方式 | 任意管理员 | 即时 |

### 9.2 变更审批流程

```mermaid
flowchart TD
    A[运营发起配置变更] --> B[填写变更内容 and 原因]
    B --> C[系统自动评估变更等级]
    C --> D{影响评估}
    D --> E[显示变更影响范围]
    E --> F{变更等级?}
    
    F -->|L1 安全| G[需 super_admin 审批]
    F -->|L2 财务| H[需 finance_admin 审批]
    F -->|L3 运营| I[需 ops_admin 审批]
    F -->|L4 常规| J[自动生效]
...
影响评估：
- 覆盖模型：128 个（所有模型的标准售价将上涨 10%）
- 受影响用户：813 人（其中活跃用户 456 人）
- 涉及代理：23 个（代理折扣价基于标准价计算）
- 预计月增收：¥8,500（基于前 3 个月均值）
- 可能风险：部分用户可能因价格上涨流失

建议：
- 通知用户：提前 7 天发送变更通知
- 生效时间：建议下月 1 日 00:00 生效，避免月内计费混乱
```
...
| 等级 | 审批超时时间 | 升级处理 |
|------|------------|---------|
| L1 安全 | 24h | 升级到 super_admin 直属上级（如有配置） |
| L2 财务 | 48h | 升级到 super_admin |
| L3 运营 | 24h | 升级到 super_admin |
| L4 常规 | N/A | 自动生效，不需要审批 |

### 9.5 配置变更生效规则

| 生效方式 | 说明 | 适用场景 |
|---------|------|---------|
| 即时生效 | 审批通过后立即生效（60 秒内） | 紧急安全策略、错误修正 |
| 定时生效 | 审批通过后指定时间生效 | 定价变更（下月 1 日）、活动发布 |
| 灰度生效 | 先对指定用户生效（灰度比例） | 限流策略变更、路由策略调整 |
| 延迟生效 | 审批后等待冷却期（默认 24h） | 全局加价率、佣金配置 |

### 9.6 变更回滚流程

```
1. 发现变更导致异常
2. 运营或 super_admin 在管理后台点击「回滚」
3. 系统展示变更前后的值对比
4. 确认回滚原因（必填）
5. 执行回滚（恢复旧值）
6. 记录回滚事件到 operation_logs
7. 根因分析后修复并重新提交变更

约束：
- 回滚操作本身也需要审批（L1/L2 级）
- 回滚后 24h 内不能再次提交相同变更（防抖动）
- 回滚不影响审计日志中的变更记录（保留完整历史）
```
...
| 审计项 | 记录内容 | 保存期限 |
|--------|---------|---------|
| 变更请求 | 发起人、时间、新旧值、影响评估、等级 | 永久 |
| 审批记录 | 审批人、审批意见、通过/拒绝、时间 | 永久 |
| 执行记录 | 执行人、执行时间、生效方式、执行结果 | 永久 |
| 回滚记录 | 回滚原因、回滚执行人、原值恢复确认 | 永久 |
...
│ ┌─ 变更申请列表 ──────────────────────────────────┐    │
│ │ 提交时间   | 变更项           | 等级 | 状态  | 操作 │    │
│ │ 14:00:00  | 全局加价率 50→60% | L2   | 待审  | [审批]│ │
│ │ 13:00:00  | 登录锁定阈值 5→3  | L1   | 待审  | [审批]│ │
│ │ 12:00:00  | 站点名称修改       | L4   | 已生效| —    │ │
│ └────────────────────────────────────────────────────┘    │
│                                                         │
│ ⚠️ 变更影响摘要（点击展开）：                                 │
│ ┌─ 全局加价率变更影响 ─────────────────────────────┐     │
│ │ 模型 128 个 | 用户 813 人 | 代理 23 个            │     │
│ │ 预计月增收 ¥8,500 | 建议提前 7 天通知用户         │     │
│ └────────────────────────────────────────────────────┘     │
│                                                         │
│ [查看历史变更] [导出变更审计日志]                           │
└─────────────────────────────────────────────────────────┘
