...
> **状态**：草案（仅需求文档）
> **优先级**：P1（隐私政策管理、服务条款版本管理）、P2（用户数据导出、渠道成本分析、合规报告、活动 ROI）
...
平台运营到一定规模后，合规法务和成本分析成为刚需。本模块覆盖：隐私政策与服务条款管理、用户数据导出（GDPR 数据可携带权）、渠道成本分析、运营活动 ROI 分析、操作合规报告。
...
分析每个渠道的成本结构、成本效率，为渠道管理和定价决策提供数据支持。
...
渠道成本分析 — 2026 年 7 月
  ...
  渠道成本占比: DeepSeek 27% / Qwen 24% / GLM 13% / OpenAI 36%
```
...
合规法务与成本分析模块涵盖隐私政策管理、服务条款版本管理、用户数据导出（GDPR）、渠道成本分析、运营活动 ROI 分析。
...
| COMP-002 | 成本分析数据延迟 | 渠道成本数据延迟到达（如 T+2 日仍未到账） | 成本分析页面显示"部分数据待更新"标记；延迟数据到达后自动刷新分析结果；未到账的数据在图表中用虚线标记为"预估" | 成本分析 | P1 |
...
| COMP-007 | 渠道成本多维度聚合异常 | 渠道成本按多维度（模型、时间、区域）聚合时出现数据不一致 | 使用 OLAP 预聚合（物化视图）保证各维度聚合结果一致性；发现不一致时刷新物化视图；告警记录不一致的范围和差值 | 成本分析 | P2 |
