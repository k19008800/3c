...
|---------|---------|---------|---------|---------|
| 渠道消费对账 | 平台 billing_logs vs 渠道账单 | 模型、Token 数、费用 | 每日 T+1 | 计费引擎 |
| 渠道结算对账 | 平台应结 vs 渠道已结 | 结算金额、笔数 | 每月结算日 | 渠道结算 |
...
| 编码 | 名称 | 含义 | 自动处理策略 |
|------|------|------|-------------|
| DIFF-001 | 平台有-对方无（漏报） | 平台有消费记录，渠道账单无 | 标记为"待确认"，自动重查 1 次 |
| DIFF-002 | 对方有-平台无（超收） | 渠道账单有，平台无对应记录 | 标记为"待核实"，需人工介入 |
