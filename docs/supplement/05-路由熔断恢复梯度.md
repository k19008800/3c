...
> **定位**：熔断器完整参数定义、恢复梯度策略（流量渐变）、渠道健康度评级、手动覆盖规则
...
多个竞争渠道的选择概率：
  渠道 A 权重 50  → 调整后 50 × 1.0 = 50
  渠道 B 权重 50  → 调整后 50 × 0.8 = 40
  渠道 C 权重 30  → 调整后 30 × 0.5 = 15
  总调整权重 = 105
...
| 类型 | 含义 | 有效期 | 优先级 |
|------|------|-------|-------|
| `manual_pin` | 强制指定渠道 | 最长 4 小时（可手动解除）| 最高 |
| `manual_block` | 屏蔽某渠道 | 最长 4 小时（可手动解除）| 次高 |
...
1. manual_pin 生效期间：
   - 该模型所有请求定向到指定渠道
   - 熔断器仍然正常工作（如果指定渠道熔断，则请求返回错误）
   - 熔断器触发不影响 manual_pin（仅返回错误，不自动切换到其他渠道）
   - 管理员可在到期前手动解除覆盖

2. manual_block 生效期间：
   - 该渠道被排除在所有路由选择之外
   - 自动熔断器对 blocked 渠道暂停评估
   - 恢复时需手动解除 block，然后进入梯度恢复

3. manual_override_weight 生效期间：
   - 手动设置权重覆盖自动健康评分权重
   - 但熔断器状态仍然生效（OPEN 状态的渠道即使手动权重高也不参与路由）
```
...
多个覆盖重叠时优先级：
  ...
  例：渠道 A 同时被 manual_pin 指定，又被 manual_block 屏蔽
  → manual_block 生效，渠道 A 不可用
  
  例：渠道 A 被 manual_pin 指定，同时渠道 B 被 manual_block
...
```
场景：渠道某个 API Key 耗尽配额，但该渠道其他 Key 仍可用
处理：
  1. 路由引擎检测到该 Key 返回 429/quota_exceeded
  2. 自动降级该 Key（不是降级整个渠道）
  3. 轮询到下一个 Key
  4. 如果该渠道所有 Key 都耗尽 → 降级整个渠道
```
...
| # | 场景 | 检测时机 | 自动处理 | 告警级别 |
|---|------|---------|---------|---------|
| R-001 | 单个渠道熔断 | 被动探测失败率 > 30% | OPEN，流量切换到其他渠道。30s 后 HALF_OPEN 试探 | P1 通知 |
| R-002 | 所有渠道熔断 | 全部渠道 OPEN | 返回 503，附带预计恢复时间。每 10s 自动健康探测 | P0 告警（电话）|
| R-003 | 某渠道 Half_Open 探测成功 | HALF_OPEN 测试 | 进入梯度恢复 Phase 1 | 无 |
| R-004 | 某渠道 Half_Open 探测失败 | HALF_OPEN 测试 | 退回 OPEN，延长等待时间（每次+15s，最多 120s）| 无 |
| R-005 | 手动覆盖到期 | cron 每小时扫描 | 自动移除过期覆盖，恢复自动路由 | 无 |
| R-006 | 渠道返回 5xx 连续 N 次 | 被动探测 | 加入熔断统计，但不立即熔断（需要达到阈值）| P2 日志 |
| R-007 | 渠道返回 4xx（非配额）| 被动探测 | 不纳入熔断统计（4xx 通常是请求问题）| P2 日志 |
| R-008 | 渠道返回 429（限流）| 被动探测 | 降级该 Key，切换到同渠道其他 Key | 无 |
