...
## 27.4 客服 SOP 与 SLA（运营视角补充）

> **P1 补充**：2026-07-30 — 客服响应 SLA、升级流程、排班策略

### 27.4.1 客服响应 SLA

| 工单类型 | 首次响应时间 | 解决时间 | 适用场景 |
|---------|------------|---------|---------|
| 紧急工单 | 15 分钟内 | 4 小时内 | 计费异常、服务中断、安全事件 |
| 高优工单 | 30 分钟内 | 8 小时内 | 功能异常、API 报错、Key 问题 |
| 普通工单 | 1 小时内 | 24 小时内 | 咨询、建议、配置修改 |
| 低优工单 | 4 小时内 | 48 小时内 | 文档反馈、功能建议 |

**SLA 计算规则：**

```
首次响应时间 = 客服首次回复时间 - 工单创建时间
解决时间 = 工单标记为已解决时间 - 工单创建时间

排除非工作时间（如配置）：
- 工作时间：09:00-18:00（周一至周五）
- 非工作时间创建的工单，首次响应时间从下一工作日 09:00 开始计算
- 紧急工单不受非工作时间限制
```
...
| 超时阶段 | 动作 | 通知对象 |
|---------|------|---------|
| SLA 超时 50% | 黄色告警，提示客服 | 客服本人 |
| SLA 超时 100% | 红色告警，自动升级 | 客服主管 |
| SLA 超时 200% | 强制升级，重新分配 | 客服主管 + 运营负责人 |
...
```
1. 交接班前 15 分钟，系统提醒交班客服
2. 交班客服梳理未完成工单
3. 系统自动将未完成的工单重新分配给接班客服
4. 接班客服确认接单
5. 交接记录写入操作日志
6. 如果接班客服未在 30 分钟内确认，工单重新进入队列
```
...
| 指标 | 定义 | 目标值 |
|------|------|-------|
| 首次响应时间 | 工单创建到客服首次回复的平均时间 | < 30 分钟 |
| 平均解决时间 | 工单创建到解决的平均时间 | < 8 小时 |
| 一次性解决率 | 无需二次回复的工单比例 | > 70% |
| 满意度评分 | 用户关闭工单后的评价 | > 4.5 / 5 |
| 日处理量 | 客服每天处理的工单数 | > 30 单 |
| 超时率 | 超出 SLA 的工单比例 | < 5% |
