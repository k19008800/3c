...
**功能定位**：将基础聊天升级为完整客服即时通讯系统（排队、状态管理、自动分配、预设回复、转工单），并配套客服绩效统计与操作审计。

**子模块说明**：
- §27.1 在线客服增强：排队机制、客服状态管理（在线/忙碌/离线）、自动分配、预设自动回复、已读/输入状态、快捷操作、聊天转工单、历史记录
- §27.2 客服绩效统计：首次响应时长、平均处理时长、解决率、满意度等指标量化团队效率
- §27.3 客服操作审计：客服操作（聊天/转单/改单）可追溯、可回滚
- §27.4 客服 SOP 与 SLA：紧急/高优/普通/低优工单的响应 SLA（15 分钟~4 小时），超时自动升级

**注意事项**：
- 客服状态切换影响自动分配：状态为"忙碌"时不再分配新会话
- 客服离线后未处理会话自动转回公共队列
- 紧急工单 SLA 倒计时 15 分钟，超时升级通知管理员
- 绩效统计按客服维度聚合，导出报表需管理员权限
- 客服操作审计日志不可删除，保留至少 180 天

**常见问题**：
Q: 为什么有用户排队但我不被分配？
A: 检查客服状态。状态为"忙碌"或"离线"时不参与自动分配。

Q: SLA 超时了会怎样？
A: 紧急工单超过 15 分钟未首次响应，自动升级通知值班管理员；超过 4 小时未解决升级到客服主管。

Q: 我可以查看自己的绩效数据吗？
A: 客服可查看自己的绩效统计，管理员可查看全团队数据。
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 设置状态 | 切换在线/忙碌/离线状态（影响自动分配） |
| 开始会话 | 接入一个新分配的客服会话 |
| 发送预设回复 | 从预设消息库中选择常用回复快速发送 |
| 转工单 | 将当前聊天转为正式工单（保留聊天记录） |
| 标记已解决 | 结束当前会话并标记为已解决 |
| 查看历史 | 查看该用户的历史聊天记录 |
| 快捷操作 | 查看用户信息/余额/调用记录（已授权时） |
| 上传附件 | 在会话中发送文件/截图附件 |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 查看审计日志 | 查看客服操作时间线（操作人/时间/内容） |
| 回滚操作 | 对可回滚的操作（如误改工单）执行回滚 |
| 导出审计记录 | 导出审计日志（CSV） |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 配置 SLA 规则 | 设置各工单类型的响应/解决时限 |
| 查看升级列表 | 查看已超时或即将超时的工单列表 |
| 手动升级 | 手动将工单升级到更高处理层级 |
