# 深化参考：§31 渠道故障演练与多环境管理

> **对应**：[`SPEC-§31-渠道故障演练与多环境管理.md`](SPEC-§31-渠道故障演练与多环境管理.md)
...
渠道宕机、配置错误、环境差异等问题的处理长期依赖人工应急。本模块提供提前验证能力：渠道故障演练、多环境配置同步、配置沙箱预览。
...
| 配置类型 | 模拟项 | 校验内容 |
|---------|--------|---------|
| 路由策略 | 权重分配变化 | 是否导致某个渠道负载异常升高 |
| 限流参数 | 阈值降低/升高 | 是否影响正常用户调用 |
| 熔断阈值 | 错误率/响应时间阈值 | 新阈值下渠道是否更易熔断 |
| 价格调整 |  模型单价变化 | 对用户端账单的影响范围 |
| 渠道切换 | 主备切换 | 是否成功切换到备用，切换时间 |
...
| # | 用例 | 预期 |
|---|------|------|
| 31.3-1 | 修改路由权重后沙箱模拟 | 显示各渠道负载变化预估 |
...
| # | 场景 | 处理方式 |
|---|------|---------|
| DRILL-001 | 演练中真实渠道也出现故障 | 演练优先终止，真实故障按正常 SOP 处理，演练标记为 cancelled |
