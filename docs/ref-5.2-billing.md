...
> **最后更新**：2026-08-20（精度口径修订：费用/金额统一 8 位小数，对齐联合评审 D-1 决议与 `PRD-模型缓存定价与缓存计量.md` §4.7）
...
| 层级 | 名称 | 说明 | 存储位置 | 优先级 |
|------|------|------|---------|-------|
| L0 | 渠道成本价 | 上游原始报价 | `vendor_models.input_price / output_price` | — |
...
计算公式：
  标准输入价 = 渠道成本输入价 × (1 + 全局加价率)
  标准输出价 = 渠道成本输出价 × (1 + 全局加价率)

示例：
  渠道成本价: input=¥0.0100, output=¥0.0300
  全局加价率: 50%
...
**精度规则**：
- 费用计算保留 8 位小数（numeric(18,8)，与 consumption_records.cost / cache_discount 列及全链路输出一致；输出 toFixed(8)，前端展示 toFixed(2)）
...
costInputPrice: numeric("cost_input_price", { precision: 18, scale: 6 }),   // L0 渠道成本
...
│ ┌─ 差异明细 ─────────────────────────────────────┐   │
│ │ 类型         | 笔数 | 金额      | 操作            │  │
│ │ 平台有-渠道无 | 23   | ¥123.45  | [核查]  [确认]  │  │
│ │ 渠道有-平台无 | 15   | ¥89.00   | [核查]  [忽略]  │  │
│ │ 金额不一致   | 7     | ¥12.30   | [查看]  [修正]  │  │
│ └────────────────────────────────────────────────┘  │
│                                                       │
│ [导出对账报告]                                         │
...
| 渠道价格变动同步延迟 | 版本控制 | 价格在配置时生效，不追溯历史 |
...
| 场景 | 处理规则 |
|------|---------|
| Token 级别精度 | 精确计算，金额保留 8 位小数（numeric(18,8)） |
| 余额精度 | 保留 2 位小数，四舍五入 |
| 批量消费精度 | 按每笔分别计算后汇总 |
| 佣金精度 | 保留 4 位小数 |

**精度溢出保护：**
1. 金额字段使用 numeric(18,8)，不用 float
