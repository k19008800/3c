...
> **⚠️ 部分废弃（2026-09）**："用户自选渠道 / `model@vendor` / 渠道选择器"部分已被 [`SPEC-模型编码化改造与去除用户供应商选择.md`](SPEC-模型编码化改造与去除用户供应商选择.md) 取代 —— 权威口径改为全局唯一**模型编码**，用户侧不再出现独立的"供应商/渠道"选择维度。本文档中"渠道化**术语统一**"部分仍有效；自选渠道相关章节仅作兼容窗口参考。
