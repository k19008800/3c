...
| # | 场景 | 触发条件 | 预期行为 |
|---|------|---------|---------|
| MDL-001 | 全部模型不可用 | 所有模型状态均为 `maintenance` 或 `offline` | ModelGrid 展示空状态插画 +「当前无可用模型」提示文字，隐藏筛选栏中的分类页签 |
| MDL-002 | 模型价格未配置 | 后台未为该模型录入平台加价价格，`input_price` 或 `output_price` 为 null | ModelCard 价格区域展示「价格待定」，点击价格区域不弹出 PriceDetailModal |
| MDL-003 | 搜索无结果 | 用户输入关键词后 API 返回 0 条匹配结果 | 展示「未找到匹配的模型，试试其他关键词」空状态，保留筛选条件供用户修改 |
| MDL-004 | 模型分组为空 | 某一分类下（如 `audio`）没有任何可用模型 | 该分类页签不显示计数，选中后 ModelGrid 展示「该分类暂无模型」空状态 |
| MDL-005 | Playground 余额不足 | 用户发送 Playground 请求时余额不足以支付预估费用 | 弹窗提示余额不足并禁止发送，底部展示充值入口 |
