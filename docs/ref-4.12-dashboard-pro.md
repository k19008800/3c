...
| `vendor_health` | 渠道健康度 | KPI API | 10s | 基准线 99% |
...
| `tech_monitor` | 技术监控 | 渠道健康/平均延迟/失败率/告警流/调用量热力图 |
...
### 8.5 VendorStatusPanel — 渠道健康状态面板
...
| 渠道管理 | ref-4.3-vendor-model.md | 大屏渠道健康面板数据源 |
...
| # | 场景 | 触发条件 | 预期行为 |
|---|------|---------|---------|
| DP-001 | 自定义图表配置错误 | 管理员在配置面板时选择了不兼容的数据源和图表类型组合（如将渠道健康状态数据加载到折线图），或图表 SQL/聚合参数无效 | 前端在拖拽配置时校验数据源-图表类型兼容性矩阵，不兼容组合标记为"❌ 不可用"并提示可选类型；服务端在保存配置时再次校验有效性，无效配置拒绝保存并返回具体错误信息 |
| DP-002 | 大数据量图表渲染性能 | 图表数据点超过 1000 个（如调用量趋势按分钟粒度展示 7 天数据），导致浏览器渲染卡顿或内存溢出 | 后端在数据查询时自动降采样：数据点 > 500 时按时间窗口聚合（取均值/最大值）；前端图表组件启用虚拟数据点渲染，超过 500 点仅渲染可视区域内的数据点；提供"原始数据"下载入口替代图内展示 |
| DP-003 | 时间范围切换数据丢失 | 用户在大屏或配置页中切换时间范围（如从 24h 切换到 7d），部分指标因数据源不支持该粒度返回空值 | 前端在切换前预查询数据源支持的时间粒度范围；不支持的时间范围对应面板显示"该时间范围暂不支持"占位；切换后保持已加载面板不闪烁抖动，新面板数据加载完成前保留上一个时间范围的最后快照 |
| DP-004 | 多个仪表盘共享冲突 | 多个管理员同时编辑同一个大屏配置，后保存者覆盖先保存者的修改 | 配置存储使用乐观锁（version 字段），后保存者若检测到版本变更则提示"配置已被他人修改，请刷新后重试"；配置编辑页设置 5 分钟编辑锁定（类似 Google Docs 的协作指示器） |
| DP-005 | WebSocket 连接断开 | 大屏页面因网络波动或服务端重启导致 WebSocket 连接中断，实时数据停止推送 | 前端自动检测连接状态，断线后立即切换到 HTTP 轮询模式（3 秒间隔），并在大屏右上角显示"实时连接已断开，切换为轮询模式"黄色提示；后台每 15 秒尝试重连，成功后切回 WS 并隐藏提示 |
| DP-006 | 大屏长时间运行内存泄漏 | 大屏页面持续运行超过 24 小时（投屏场景），由于实时数据流不断累积或图表动画未清理导致浏览器内存持续增长 | 前端实现数据窗口滑动：实时数据流仅保留最近 1 小时的事件，超出部分自动丢弃；图表数据点超过上限时自动裁剪旧数据；设置每 30 分钟自动清理一次内部缓存和未使用的订阅；用户可点击"刷新页面"重建大屏状态（不丢失屏配置） |

### 异常流程

| 场景 | 恢复策略 |
|------|---------|
| KPI 数据查询接口超时 | 卡片显示"--"占位值，保留上次有效数据；超时接口自动重试 1 次后若仍失败则跳过该卡片刷新 |
| 告警事件流拥塞（突发大量告警） | 事件流限速：每秒最多展示 10 条，超出部分合并为"+N 条同类告警"折叠按钮 |
| 大屏全屏 API 被浏览器限制 | 提示用户手动点击浏览器全屏按钮（F11），或使用系统提供的全屏替代方案 |
| 屏幕轮播时某屏面板加载失败 | 跳过该屏，展示 3 秒"数据加载中…"后自动切换到下一屏，失败屏标记为"需手动刷新" |
