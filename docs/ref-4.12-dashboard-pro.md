...
| # | 场景 | 触发条件 | 预期行为 |
|---|------|---------|---------|
| DP-001 | 自定义图表配置错误 | 管理员在配置面板时选择了不兼容的数据源和图表类型组合（如将供应商健康状态数据加载到折线图），或图表 SQL/聚合参数无效 | 前端在拖拽配置时校验数据源-图表类型兼容性矩阵，不兼容组合标记为"❌ 不可用"并提示可选类型；服务端在保存配置时再次校验有效性，无效配置拒绝保存并返回具体错误信息 |
