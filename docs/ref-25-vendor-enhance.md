...
在现有渠道管理体系（§4.3 渠道与模型管理）基础上，从渠道视角和管理视角补充 4 项核心能力：结算对账、渠道公告、性能排行、自助结算。其中自助结算提供渠道登录端自行申请结算的流程。
...
POST   /api/v1/vendor/settlements/:id/confirm     — 渠道确认
POST   /api/v1/vendor/settlements/:id/dispute     — 渠道发起争议
  body: { reason }
```
...
| # | 用例 | 预期 |
|---|------|------|
| 25.1-1 | 管理员对账确认 | 查看渠道结算数据、核对各模型明细 |
| 25.1-2 | 渠道确认结算 | 在渠道门户确认结算单 |
| 25.1-3 | 争议处理 | 渠道发起争议→管理员审核→调整或驳回 |
...
// 渠道端
GET    /api/v1/vendor/announcements               — 公告列表（渠道端，仅自己的）
...
| # | 用例 | 预期 |
|---|------|------|
| 25.2-1 | 管理员发布公告 | 指定目标渠道、分类、优先级 |
| 25.2-2 | 渠道查看公告 | 显示所有面向自己的公告，置顶优先 |
...
```
渠道健康检查 → vendor_health_check_records → F1 评分聚合 → 排名
```
...
| 并发支持能力 | 10% | 渠道上报/实测 | 月度 |
| 合规与稳定性 | 5% | 运营手动评估 | 月度 |

### API 接口

```
GET    /api/v1/admin/vendor/ranking               — 渠道综合排行
  params: { periodType: 'weekly'|'monthly', page, limit }
GET    /api/v1/admin/vendor/ranking/:vendorId     — 某渠道评分详情
GET    /api/v1/vendor/ranking                     — 渠道端查看自己排名+维度评分
```
...
export const vendorSelfSettlementRequests = pgTable("vendor_self_settlement_requests", {
  ...
  requestType: varchar("request_type", { length: 20 }).default("auto"),
    // 'auto' | 'manual'  — auto: 系统按账期自动生成员; manual: 渠道手动申请
...
| 条件 | 说明 |
|------|------|
| 最低结算金额 | 渠道累计待结算金额 ≥ ¥100 |
| 结算周期 | 距上次结算至少 30 天 |
| 渠道状态 | 渠道状态为 active（非暂停/下线）|
| 争议工单 | 无未处理争议 |
| 合作时长 | 入驻 ≥ 30 天 |
| 数据完整 | 最近 7 天无计费异常 |
...
| # | 用例 | 预期 |
|---|------|------|
| 25.4-1 | 渠道查看结算资格 | 显示是否满足条件，不满足时提示原因 |
| 25.4-2 | 渠道发起结算申请 | 系统计算预估金额，提交审核 |
| 25.4-3 | 管理员审核申请 | 查看结算数据→批准或拒绝 |
| 25.4-4 | 申请被拒 | 渠道查看拒绝原因，可调整后重新申请 |
...
| VEN-002 | 渠道在结算期间发生价格变更 | 结算按合同单价计算，价格变更从下一周期生效 |
| VEN-003 | 公告发布后渠道已读率低于设置阈值 | 管理员收到通知，可选追加公告或单独联系 |
| VEN-004 | 自助结算金额远低于渠道期望 | 展示详细计算过程和调用明细供核对 |
| VEN-005 | 结算对账数据量过大（百万级记录） | 使用异步汇总任务生成结算单，不可前端实时计算 |
| VEN-006 | 渠道同时申请多个账期 | 仅允许申请距最后结算日最近的一个账期 |
| VEN-007 | 公告发布时渠道为 inactive | 不发送通知；渠道重新激活后看到历史公告 |
...
§25 渠道增强:
  ...
  ├── §25.2 渠道公告: vendorAnnouncements → 通知服务 → 站内+邮件
