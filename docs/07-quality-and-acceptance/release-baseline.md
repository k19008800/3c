...
| 18（2026-09-06 测试增强） | verify R3 + `cd e2e && pnpm test`（R4） | ~2min | 17 + 37 | 0 | ✅ **测试增强复验**：`scripts/test-integration.cjs` 第 8 项注释修正（wanwu）；`fullflow.spec.ts` ③ 补 `mock !== true` 断言 → verify R3 17/17、E2E R4 37/37（服务重启后全绿；R2/R3 偶发 flake 为 dev server 长时运行的环境不稳定，隔离均通过，非代码回归） |
| 19（2026-09-06 #17/#26 收口） | 独立全新库迁移演练 + paid/dispute 收口回归 | ~4min | 1337 + 37 | 0 | ✅ **#17 关闭**：独立全新库 `threecloud_migration_drill_20260906` drizzle 0000–0016（17 条）+ manual 0017–0036（22 条 applied/checksum 完整/post-check 通过/幂等重跑全 skip），演练库已清理（日志 12-migration-drill-*）。**#26 paid/dispute 收口**：迁移 0037 加列（paid_at/payment_reference/dispute_reason/disputed_at）+ `POST /:id/paid`、`POST /:id/dispute`（挂操作级 2FA）+ confirm 支持解决争议 + 状态命名统一 generated + 前端 `.list`→`.items`；专测 15/15（2FA 7 + ops 8）+ vendor-settlement 23/23；**API 全量 92 文件/1337 测试全绿**、typecheck/lint/build 通过、**E2E R5 37/37** |
...
`ready`（2026-09-06 全量重跑，R2 修复 + R3 测试增强后）：代码门禁**全绿**（typecheck/lint/API 1327/web-console 46/build 4 端/E2E 37）；verify **17/17**（R1 16/17 → R2 修复后全绿，chat 走真实 wanwu 上游，计费 ¥0.000052 精确）。修复动作为本地渠道数据：补 `vendor_pricing`（id=4128，supplier_model 2317，default 档 ¥0.004/¥0.012 per 1K，覆盖 wanwu 成本 ¥0.002/¥0.008 约 1.5×）+ 修正 `supplier_models` 2317 `platform_model`（`deepseek-v4-flash` → `DeepSeek-V4-Flash-0731`，wanwu 渠道实测仅认此名），详见 `ops/baseline-2026-09-06/REPORT.md` §4 与 open-issues #27（已关闭）。**测试增强（#27 遗留建议一并处理）**：verify 脚本注释修正 + E2E ③ `mock !== true` 断言，R3 verify 17/17、R4 E2E 37/37 复验全绿。**#17/#26 收口（2026-09-06 三次收口）**：#17 独立全新库迁移演练（drizzle 0000–0016 + manual 0017–0036，幂等重跑全 skip）已真实执行关闭；#26 前端 paid/dispute 按钮后端缺口已收口关闭（迁移 0037 + paid/dispute 端点 + 操作级 2FA + 状态机 + 前端 `.items` 契约对齐），API 全量 **1337/1337**、E2E **R5 37/37**、typecheck/lint/build 全过。**非阻断项**：T-04 长线项（Redis 分布式锁/异步补偿/outbox/外部账单导入/跨系统对账/正式结算周期）。