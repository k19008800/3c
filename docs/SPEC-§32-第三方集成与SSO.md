...
**页面名称**：第三方集成与 SSO 单点登录

**适用角色**：
- 管理员（运维/IT）：配置全局 Webhook、SSO 集成、企业通讯录登录
- 企业客户：对接企业身份系统登录平台

**功能定位**：将平台事件推送到外部系统，对接企业身份系统实现 SSO，支持企业微信/钉钉/飞书账号登录。

**子模块说明**：
- §32.1 全局 Webhook 出站：所有订单/事件变更推送到外部系统，支持签名验证和重试
- §32.2 SSO 单点登录：对接 LDAP/OIDC/SAML，企业客户用公司账号登录
- §32.3 企业通讯录账号登录：企业微信/钉钉/飞书扫码登录管理后台

**注意事项**：
- Webhook 配置变更使用软切换（旧 URL 在 5 分钟过渡期内继续接收）
- SSO 配置变更影响所有使用该 SSO 的登录用户，建议分阶段迁移
- 通讯录同步失败会自动使用最后一次成功同步的数据
- OAuth 回调 URL 严格匹配，不支持通配符

**常见问题**：
Q: SSO 提供商会影响本地登录吗？
A: 当 SSO IdP 不可用时，自动降级到本地密码登录（如果启用了本地登录）；纯 SSO 模式显示维护公告。

Q: Webhook 投递失败会重试吗？
A: 会。连续 5 次签名失败会锁定该 Webhook 目标 URL。一般投递失败按指数退避重试最多 3 次。

Q: 支持多个 SSO 提供商同时配置吗？
A: 支持。用户可以设置默认 SSO 提供商，未设置时显示选择页面。
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 创建 Webhook | 配置目标 URL、签名密钥、订阅事件类型列表 |
| 编辑 Webhook | 修改 Webhook 配置（使用软切换，旧 URL 继续接收 5 分钟） |
| 测试投递 | 发送测试事件到目标 URL，验证连通性和签名 |
| 查看投递日志 | 查看 Webhook 的历史投递记录和状态 |
| 重试投递 | 对失败的 Webhook 投递手动触发重试 |
| 暂停/启用 | 暂时停止或恢复 Webhook 投递 |

**§32.2 SSO 单点登录**

| 按钮/操作 | 帮助说明 |
|----------|---------|
| 添加 SSO 提供商 | 配置 IdP 类型（OIDC/SAML/LDAP）、元数据 URL、属性映射规则 |
| 编辑 | 修改 SSO 提供商配置 |
| 测试登录 | 模拟 SSO 登录流程，验证配置是否正确 |
| 查看登录日志 | 查看 SSO 登录成功/失败记录 |
| 禁用 SSO | 临时禁用 SSO 提供商（用户回退到本地登录） |
| 设置默认 | 设置用户的默认 SSO 提供商（节省选择步骤） |

**§32.3 企业通讯录账号登录**

| 按钮/操作 | 帮助说明 |
|----------|---------|
| 同步通讯录 | 手动触发从企业 AD/LDAP 同步通讯录 |
| 查看映射 | 查看企业账号与平台账号的映射关系 |
| 处理冲突 | 手动处理邮箱冲突的账号映射 |
