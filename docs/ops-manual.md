...
| 等级 | 示例 | 响应时间 | 处理人 |
|------|------|---------|--------|
| 🔴 紧急 | 渠道 API Key 泄露、用户数据泄露 | 15 分钟内 | super_admin |
...
| 日毛利率 | (日消费总额 - 日渠道成本) / 日消费总额 × 100% | 计算值 |
...
| 渠道异常 | 健康检查 + 熔断器 | 自动切换 + 通知用户 | ref-4.3-vendor-model.md §7 |
...
| 场景 | 操作 | 参考 |
|------|------|------|
| 渠道健康检查异常 | 确认检查结果，决定是否切换 | ref-4.3-vendor-model.md §7.3 |
| 渠道批量异常 | 一键降级，通知受影响用户 | ref-4.3-vendor-model.md §7.4 |
| Key 池耗尽 | 紧急补充 Key 或切换渠道 | ref-4.3-vendor-model.md §7.2 |
| 渠道入驻审核超时 | 升级通知运营主管 | ref-4.3-vendor-model.md §8.1.2 |
| 渠道价格变更审批 | 按变更级别走审批流程 | ref-4.3-vendor-model.md §8.2.1 |
| 渠道状态切换 | 使用管理后台弹窗操作 | ref-4.3-vendor-model.md §5.4 |
