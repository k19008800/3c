...
> **状态**：review（历史来源，未完成实现核验）
> **版本**：v1.0.0
> **上游 ADR**：ADR-0024
> **优先级**：P0（角色管理增强、用户权限一览）、P1（权限变更审计、API Key 细粒度权限）、P2（权限模板、权限自检）
>
> **实现边界**：本文的动态角色、权限覆写和角色管理接口不得标记为 implemented，除非存在对应 Schema、路由和测试证据。
...
>
> **适用角色**：`super_admin`（最高权限）；`admin` 仅按具体权限点授权，不因角色名自动获得角色管理权限
>
> **核心操作**：
- 配置角色和权限策略
- 管理用户角色分配
...
历史设计曾规划基于 bitset 的动态角色权限引擎（`admin_roles`、`userRoleAssignments`、`userPermissionOverrides`），但当前是否已具备对应 Schema、路由和测试证据须单独核验；本文不得据此宣称功能已实现。
...
规划中的角色管理页面包括权限树可视化勾选、角色详情查看、创建/编辑/删除角色和角色下用户列表；在对应 Schema、路由、权限守卫和测试完成前，均标记为 `planned`。
...
```
// 已有
GET    /api/v1/admin/users/:id/permissions           — 规划：查询权限覆写
PUT    /api/v1/admin/users/:id/permissions           — 规划：全量替换权限覆写
DELETE /api/v1/admin/users/:id/permissions           — 规划：清除权限覆写
...
| PERM-004 | 权限模板冲突 | 用户同时被分配了多个权限模板，模板间存在权限冲突 | 角色权限先取并集；最终按 ADR-0024 执行显式 deny → 管理员强制策略 → 显式 grant → 角色权限并集 → 默认最小权限；无明确授权默认拒绝 | 用户权限 | P1 |
