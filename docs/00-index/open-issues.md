...
17. **~~迁移 runner 加固（代码已落地，演练证据待补）~~：已关闭（2026-09-06 #17 收口）**。`api/run-manual-migrations.cjs` 已按 ADR-0007/0026 加固：元数据记录 checksum/状态/错误/耗时，使用 PostgreSQL advisory lock 单执行者，checksum 变化报警并失败，失败即停且不自动回滚；针对性纯逻辑测试已通过。**独立全新库演练已真实执行（2026-09-06）**：新建 `threecloud_migration_drill_20260906`（全空库）→ drizzle journal `0000–0016` 应用成功（17 条记录）→ manual `0017–0036` 全部 applied（22 条，checksum 完整，post-check 通过：`consumption_records`/`balance_transactions` 分区、`users.language`、checksum 列）→ 92 张 public 表 → **幂等重跑 22 个迁移全部 `already applied (checksum ok)` 跳过、post-check 仍通过**（无重复 DDL）→ 演练库已清理。备份恢复证据已于 #26 收口时真实 PASS（`evidence/finance/v0.1.0/restore/`，2026-09-06）。日志：`ops/baseline-2026-09-06/12-migration-drill-*.log`。
...
- **2026-09-06 三次收口（paid/dispute 后端缺口已闭环，#26 全部关闭）**：
  - **迁移 0037** `vendor_settlements` 加列：`paid_at`、`payment_reference`、`dispute_reason`、`disputed_at`（`api/src/db/migrations/0037_vendor_settlements_payment.sql`，已加入 runner MIGRATIONS 并在本地库应用）。
  - **服务层**（`api/src/services/finance/vendor-settlement.ts`）：新增 `markSettlementPaid`（confirmed → paid，写 paid_at/payment_reference，幂等）、`markSettlementDispute`（generated/draft → disputed，写 dispute_reason/disputed_at，幂等）；`confirmSettlement` 扩展支持 disputed → confirmed（解决争议，清空争议字段）；**状态命名统一**：generate 产出 `generated`（历史 `draft` 兼容确认/争议）。
  - **路由**：`POST /admin/vendor-settlements/:id/paid`、`POST /:id/dispute` 新增，均挂 `[adminAuth, requireOperation2fa]`。
  - **前端**：AdminVendorSettlementsPage 列表字段 `.list` → `.items`（对齐 ADR-0030 分页契约与后端实现，修复列表空渲染导致按钮不可达的既有缺陷）；paid/dispute/confirm 按钮与 2FA 弹窗原有实现直接可用。
  - **测试**：`admin-settlement-2fa.test.ts` 扩至 7/7（paid/dispute 2FA 契约：缺令牌 403/带令牌 200/重放 403）；新增 `admin-vendor-settlement-ops.test.ts` 8/8（状态机 + 字段落库 + list/detail 返回 + status 过滤）；`test/vendor-settlement.test.ts` 状态断言 `draft`→`generated` 同步。**回归**：API 全量 **92 文件 / 1337 测试全绿**、typecheck/lint/web-console build 通过、E2E 全量 **37/37 全绿**。

> 以上 #22–26 为真实执行结果或 accepted ADR 对照实现后显示的生产阻断/失败项。#22/#25/#23/#24/#26 均已关闭（2026-09-06）；备份恢复门禁已真实执行 PASS（证据 `evidence/finance/v0.1.0/restore/`，见 release-baseline）。2026-09-06 二次收口：候选发布提交 `a7958ad` 已确定（可发布源码与 evidence/audit/docs/test-reports 分离后提交，API 1327/1327 + build patched 0 + 前端 build 通过）；T-04 补账/核销事务证据已闭环（diffs 端点 2FA + 专测 5/5）。发布基线整体 `ready`（见 release-baseline）；非阻断登记项：#17 独立全新库迁移演练、前端 paid/dispute 按钮后端缺口、T-04 长线项（Redis 分布式锁/异步补偿/outbox/外部账单导入/跨系统对账/正式结算周期）。**2026-09-06 三次收口（全部剩余登记项已闭环）**：#17 独立全新库迁移演练已真实执行关闭；前端 paid/dispute 按钮后端缺口已收口关闭（迁移 0037 + paid/dispute 端点 + 2FA + 状态机，API 1337/1337 + E2E 37/37）；T-04 长线项仍为开放项（不阻塞发布准入）。

---

### 2026-09-06 追加未决（候选提交 `8d3cad7` 全量基线重跑时登记，见 `ops/baseline-2026-09-06/REPORT.md`）

27. **✅ 已关闭（2026-09-06 R2）— verify chat 真实上游失败（本地渠道数据缺口，代码无回归）**：全量重跑 verify R1 16/17，唯一失败 `Chat completions (real upstream)`——`POST /v1/chat/completions`（model `DeepSeek-V4-Flash-0731`）返回 `{"mock":true}`。**根因（两层，均已修复）**：① 数据层——`api/src/services/upstream/routing.ts` `selectChannel` 以 `vendor_pricing.status='active'` 为必要条件（innerJoin），而本地库 `vendor_pricing` 中 wanwu（supplier 2633 / supplier_model 2317）无定价记录 → 路由 null → mock 回退；② 渠道映射层（修复①后暴露）——上游请求 model 用 `platform_model` 转发（`api/src/routes/chat.ts`），而 2317 的 `platform_model='deepseek-v4-flash'` 与 wanwu 实际模型名 `DeepSeek-V4-Flash-0731` 不符 → wanwu 503 `model_not_found`。**修复动作（本地库数据，附证据）**：补 `vendor_pricing` id=4128（supplier_model_id=2317，pricing_group=default，¥0.004/¥0.012 per 1K，覆盖 wanwu 上游成本 ¥0.002/¥0.008 约 1.5×，与本地 default 档既有价格点一致且 < ¥10 单位校验红线）+ `UPDATE supplier_models SET platform_model='DeepSeek-V4-Flash-0731' WHERE id=2317`。**验证**：直连 chat → 200 真实 completion（mock 消失）；R2 verify **17/17**，计费 ¥0.000052（10 input×0.004/1000 + 1 output×0.012/1000）与余额扣减（¥10→¥9.999948）精确吻合。**遗留建议项（已于 2026-09-06 一并处理）**：① `scripts/test-integration.cjs` 第 8 项注释已修正（可用渠道=wanwu 非天翼云）；② `e2e/tests/fullflow.spec.ts` ③ 已补 `mock !== true` 断言（修复后全量 E2E 37/37 全绿，含真实上游 fullflow ③）。
