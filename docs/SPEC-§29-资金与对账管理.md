...
| 29.3 | 对账差异处理工作台 | P0 | 集中处理平台-渠道/平台-代理商对账差异 |
...
财务人员查看平台上每一笔资金的进出明细，涵盖用户充值、用户消费、代理商佣金结算、渠道结算、退款、提现、平台内部调账等所有资金变动。
...
资金流水
  ...
  │10:00 │FL2026  │渠道   │DeepS  │—      │¥2,500 │¥10,802│
...
├── 已结算给渠道: ¥XXX,XXX.XX   ← 已支付给渠道
├── 待结算给渠道: ¥XXX,XXX.XX   ← 未到结算周期的渠道费用
├── 已发放代理佣金: ¥XXX,XXX.XX   ← 已发放给代理
├── 待结算代理佣金: ¥XXX,XXX.XX   ← 代理已计提但未提现/未结算
├── 平台毛利:     ¥XXX,XXX.XX     ← 用户消费 - 渠道成本 - 佣金
└── 平台毛利率:   XX.X%           ← 毛利 / 用户消费
```
...
集中处理平台与渠道之间、平台与代理商之间的对账差异。当自动对账发现不一致时，财务人员在工作台中查看差异详情、分析原因、标记处理。
...
对账差异处理
  ...
  ├──────┼──────────┼──────────┼──────────┼────────┼──────────┤
  │07/28 │渠道    │ DeepSeek │ +¥23.50  │待处理  │ [处理]   │
  │07/28 │渠道    │ OpenAI   │ -¥12.00  │待处理  │ [处理]   │
  │07/27 │代理佣金  │ 张三     │ +¥5.00   │已处理  │ [查看]   │
  │07/27 │渠道    │ GLM      │ ¥0.00    │已对平  │ [查看]   │
  └──────┴──────────┴──────────┴──────────┴────────┴──────────┘
...
支持美元/港币等多币种结算，自动汇率换算，适配海外渠道结算需求。
...
| 计费日志 → 渠道结算 | SUM(实际用户消费) - SUM(渠道消费) == 平台毛利 | 月结 | 模型维度 | 偏差 ≥ ¥100 |
...
| 渠道结算偏差 | 平台毛利率异常（低于目标值 5% 以上） | 日报 -> 邮件 | 财务负责人 |
...
| 毛利率 | (日消费总额 - 日渠道成本) / 日消费总额 × 100% | 08:00 |
