...
> **⚠️ 已废止（2026-09）**：本文档规划的"用户自选渠道 + `model@vendor` 二维设计"已被 [`SPEC-模型编码化改造与去除用户供应商选择.md`](SPEC-模型编码化改造与去除用户供应商选择.md) 取代。权威口径改为：以全局唯一**模型编码**为 API `model` 权威值，**纯编码、无自动、无兼容窗口**，用户侧不再出现独立的"供应商/渠道"选择维度。本文档仅作历史留档，不得作为新集成依据。
