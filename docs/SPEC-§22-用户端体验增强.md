...
POST /api/v1/me/notifications/preferences/reset  鈥?鎭㈠榛樿
  鍝嶅簲锛歿 success: true }  // 閲嶇疆涓洪粯璁ゅ€?```
> 📌 **路径说明**：文档此前写为 `me/preferences/notifications`，代码实际为 `me/notifications/preferences`（`notification-subscriptions.ts`，含 settings/subscriptions 子接口），已于 2026-07-31 统一。`reset`（恢复默认）接口代码未实现，待开发。
...
**功能定位**：面向用户端的体验增强：注册引导、仪表盘增强、Playground 在线调试、Webhook 配置、第三方登录、通知偏好、API Key 操作日志、邀请机制、用量对比、错误码自助排查、批量操作、统一数据导出。

**子模块说明**：
- §22.1 注册引导与 Onboarding：新用户分步引导，断点续进，可跳过
- §22.2 Dashboard 体验增强：成本预估、异常警告、账单周期、实时活动流、数据导出
- §22.3 用户端 Playground：零代码在线调试 API（默认超时 60s，可调至 300s）
- §22.4 Webhook 配置：事件主动推送到用户服务器（重试耗尽自动禁用并通知）
- §22.5 第三方 OAuth 登录：微信/GitHub 等第三方账号登录与绑定
- §22.6 通知偏好增强：精细控制各类通知渠道
- §22.7 API Key 操作日志：Key 的创建/删除/权限变更记录（保留 90 天）
- §22.8 用户邀请机制：邀请码/链接邀请新用户
- §22.9 用量对比分析：跨时间段用量对比（按天/月预聚合）
- §22.10 错误码自助排查：错误码说明与排查指引
- §22.11 批量操作：批量创建 Key、批量设置预算等
- §22.12 统一数据导出：用户数据一键导出

**注意事项**：
- 数据导出并发上限 5 个任务，单用户每 30 分钟 1 次
- Webhook 重试耗尽后自动禁用，需手动重新启用
- 第三方账号已被绑定时会提示解绑引导流程
- Playground 调用默认超时 60 秒
- OAuth state 参数一次性使用，并发请求仅第一个生效

**常见问题**：
Q: Onboarding 中途退出会丢失进度吗？
A: 不会。系统保存引导进度，下次登录从断点恢复；也可跳过，随时在设置中重新启动。

Q: Webhook 为什么被禁用了？
A: 发送失败重试（默认 3 次）耗尽后自动标记为 DISABLED，需手动重新启用。

Q: 用量对比查询很慢？
A: 用量数据按天/月预聚合，单次查询最长 15 秒，超时建议缩小范围或使用导出。
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 查看成本预估 | 查看本月预计消费（基于当前用量趋势） |
| 查看异常警告 | 查看余额不足/调用失败率上升等异常警告 |
| 账单周期 | 切换查看不同账单周期的消费 |
| 实时活动流 | 实时查看 API 调用活动记录 |
| 导出数据 | 导出仪表盘数据（CSV） |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 发起测试调用 | 在 Playground 中执行一次 API 测试调用 |
| 中止 | 中止正在执行的超时调用 |
| 调整超时 | 修改调用超时时间（60s~300s） |
| 历史记录 | 查看最近的测试调用记录 |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 创建 Webhook | 配置回调 URL、签名密钥、订阅事件 |
| 测试发送 | 发送测试事件验证配置 |
| 重新启用 | 重新启用被自动禁用的 Webhook |
| 查看投递记录 | 查看历史事件投递状态 |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 绑定第三方账号 | 将微信/GitHub 等账号绑定到当前 3cloud 账号 |
| 解绑 | 解除第三方账号绑定 |
| 登录 | 使用第三方账号快速登录 |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 查看操作日志 | 查看 Key 的创建/删除/权限变更记录 |
| 导出日志 | 导出 Key 操作日志（CSV） |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 批量创建 Key | 一次创建多个 API Key |
| 批量设置预算 | 批量设置多个 Key/项目的预算限额 |
| 批量删除 | 批量删除选中的 Key（需确认） |

**§22.12 统一数据导出**

| 按钮/操作 | 帮助说明 |
|----------|---------|
| 发起导出 | 申请导出用户数据（每 30 分钟 1 次） |
| 查看导出记录 | 查看导出任务状态和下载链接 |
