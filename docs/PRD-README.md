...
> **定位**：这是运营级 PRD 的来源总纲与导航，不是全库唯一准入入口。正式开发/验收准入以 [`00-index/README.md`](00-index/README.md) 及其 [`governance-policy.md`](00-index/governance-policy.md)、[`feature-package-matrix.md`](00-index/feature-package-matrix.md)、[`document-inventory.md`](00-index/document-inventory.md) 为准。
>
> **状态**：各章节是来源/需求资料；只有登记并达到治理政策要求的正式文档，才可作为开发与验收依据。
...
> 🎯 **核心流程泳道图**：[`docs/flowcharts/`](flowcharts/) — 6 份泳道图覆盖充值、提现双审、实名审核、渠道状态切换、自动对账、代理晋升全流程。
...
| | `ref-4.3-vendor-model.md` §7 | 渠道异常降级、上下线影响、Key 耗尽 |
...
### 🔶 模型编码化改造（2026-09 新增，取代"用户自选渠道"）

**用户侧"供应商/渠道选择"移除，改为以"模型编码（model_code）"为唯一路由键。**

| 文档 | 状态 | 说明 |
|------|------|------|
| [`PRD-模型编码化改造与去除用户供应商选择.md`](PRD-模型编码化改造与去除用户供应商选择.md) | 待评审 | 需求 PRD |
| [`SPEC-模型编码化改造与去除用户供应商选择.md`](SPEC-模型编码化改造与去除用户供应商选择.md) | 待评审 | 实现规格 |
| [`user-vendor-selection.md`](user-vendor-selection.md) | ⚠️ 已废止 | 旧 `model@vendor` 二维设计，仅保留兼容窗口 |
| [`SPEC-渠道化术语统一与用户自选渠道定价.md`](SPEC-渠道化术语统一与用户自选渠道定价.md) | ⚠️ 部分废弃 | "术语统一"有效；"自选渠道"部分被取代 |

> **权威语义**：`model_code` 一对一绑定 `supplier_models`（供应商 + platform_model），是用户 API `model` 参数的权威值；用户侧不出现独立"供应商/渠道"选择维度。

### §25 vs ref-4.10（渠道增强 vs 渠道自助管理）

| 维度 | §25 SPEC | ref-4.10-vendor-self-service.md |
|------|----------|--------------------------------|
| 视角 | **渠道自助端**（渠道登录后查看结算/通知/排行榜/自助结算） | **管理后台端**（管理员配置/管理渠道接入） |
| 内容 | 结算对账查看、公告通知、性能排行榜（渠道视角） | 渠道入驻、模型配置、Key 管理、自助仪表盘（管理视角） |
