...
| # | 场景 | 触发条件 | 预期行为 |
|---|------|---------|---------|
| OT-001 | 时间线事件积压 | 用户操作频率极高（如每分钟调用 API 数百次），导致时间线在短时间内生成大量事件记录，加载和渲染时数据量过大 | 时间线按天分页，每页最多 50 条事件；超过 50 条时展示"+N 条待加载"折叠按钮；事件按优先级折叠合并：同类型连续事件（如同模型大量调用）合并为一条聚合节点，标注"连续调用 N 次" |
| OT-002 | 事件排序冲突 | 两个或多个事件在同一毫秒内发生（如系统自动操作和管理员手动操作时间戳完全一致），导致时间线上排序不确定 | 以数据库自增 ID 作为二级排序字段（同毫秒内按 ID 升序）；系统操作和管理员操作分别标记图标颜色（🔧 系统 / 👤 管理），同一时刻的事件按类型分组展示 |
| OT-003 | 批量事件折叠显示异常 | 运营人员在短时间内批量处理操作（如批量审核、批量导出），时间线上同时涌入了 N 条高度相似的独立事件 | 相同 action + 操作人 + 时间窗口 ≤ 60s 的连续事件自动折叠为聚合节点，标注"批量 N 条操作"，点击展开详细列表；折叠/展开状态持久化到 localStorage |
| OT-004 | 跨时区时间显示 | 管理员位于不同时区（如北京时间 +8 和纽约时间 -4），查看同一时间线时事件时间信息混乱 | 所有时间数据统一以 UTC 存储，前端根据用户浏览器时区自动转换显示；时间线顶部显示"时区：UTC+8"提示，提供手动切换时区选项；用户鼠标悬停时间戳时 tooltip 显示 UTC 标准时间 |
| OT-005 | 实时流模式 WebSocket 断连 | 全局时间线实时流模式中 WebSocket 断线后，后台收集的事件在重连后一次性批量插入 | 断线期间在时间线顶部显示"已断开，正在缓存事件"横幅；重连后按时间顺序批量插入断线期间缓存事件（每秒最多插入 10 条以避免界面抖动）；用户可点击"跳过历史，跳转实时"忽略缓存事件 |
| OT-006 | 敏感操作二次确认被强行关闭 | 二次确认弹窗被用户强行关闭（点击弹窗外的遮罩层或按 ESC），敏感操作未经确认即执行 | 弹窗关闭（非确认/取消）视为操作取消，不执行后续敏感操作；用户关闭弹窗后按钮恢复可用状态，可重新点击触发二次确认流程；操作日志中记录"敏感操作二次确认弹窗被关闭（未确认）" |

### 异常流程

| 场景 | 恢复策略 |
|------|---------|
| 时间线导出 PDF/CSV 超过数据量限制 | 导出任务转为异步后台执行，完成后发送站内信下载链接；导出数据量超过 10 万条时提示"数据量过大，仅导出最近 10000 条" |
| 风险评分计算依赖的外部数据源异常 | 跳过异常检测规则中依赖该数据源的规则项，用已有规则计算部分评分，前端标记"部分异常检测不可用" |
| 时间线虚拟滚动时筛选条件变更 | 重置滚动位置到顶部并重新加载数据，保持当前时间范围筛选状态不变 |
