...
| [`SPEC-§18-用户端体验增强.md`](SPEC-§18-用户端体验增强.md) §18.6 账号安全中心 | 设备管理页面（20.3）+ 2FA 设置页面（20.2）+ 异常登录展示（20.5）| 在安全中心新增子页面 |
| §2 登录记录 | 异常检测前端展示（20.5）| 重新渲染 login-history 页面，添加风险标记和交互按钮 |
| [`SPEC-§14-错误码规范.md`](SPEC-§14-错误码规范.md) | 新增错误码（见下方）| 在 §14 错误码参考页中追加 |

### 边界声明：§20.4（用户侧）与 §30.4（管理侧）Key 权限控制

> **本模块（§20.4）** 定位为**用户侧自服务**——用户对自己的 API Key 设置模型范围、IP 白名单、域名限制、每日额度等。
> **§30.4** 定位为**管理侧全局管控**——管理员可为所有用户 Key 设置全局默认权限、强制覆盖策略、权限模板。

| 维度 | §20.4（用户侧） | §30.4（管理侧） |
|------|----------------|----------------|
| 操作者 | 普通用户 | 管理员（super_admin / security_ops） |
| 范围 | 仅对自己创建/管理的 Key | 可查看和管理所有用户 Key |
| 配置方式 | 用户 Key 管理页面 → 权限 Tab | 管理后台 → 权限管理 → API Key 权限 |
| API 端点 | `PUT /api/v1/me/api-keys/:id/permissions` | `PUT /api/v1/admin/api-keys/:id/permissions` |
| 模板支持 | 用户从已有 Key 复制权限 | 管理员创建权限模板批量应用 |
| 强制覆盖 | 用户侧配置无强制覆盖能力 | 管理员可强制覆盖用户侧配置 |
| 数据源 | 同一张 `api_keys` 表扩展字段 | 同一张 `api_keys` 表扩展字段 + `key_permission_templates` |
| **冲突规则** | 用户侧 + 管理侧配置取交集（更严格的一方生效） | 管理侧配置为基线，用户侧可缩小不可扩大 |

**实现注意**：两端共用 `api_keys` 表扩展字段（`ip_whitelist`、`domain_whitelist`、`daily_call_limit`、`daily_token_limit`、`daily_cost_limit`），网关中间件统一校验，不分来源。
...
| 20.2 双因素认证 | P0 | auth 服务 + 通知服务 + [`SPEC-§14-错误码规范.md`](SPEC-§14-错误码规范.md) |
...
**功能定位**：从用户视角补全平台的安全控制与预算管理能力：消费预算熔断、全链路双因素认证、设备级会话管理、API Key 细粒度权限控制、登录异常检测展示。

**子模块说明**：
- §20.1 用户消费预算设置：月度消费预算与熔断（超限自动拦截请求，返回 QUOTA_EXCEEDED）
- §20.2 双因素认证（2FA）：用户端 + 管理后台双设定，支持 TOTP 等
- §20.3 设备管理：查看/吊销已登录设备，会话级安全控制
- §20.4 API Key 权限控制（用户侧）：Key 级权限策略（模型/IP/操作范围），最小权限原则
- §20.5 登录异常检测前端展示：异地登录、异常设备登录提示，用户感知安全状态

**注意事项**：
- 预算检查优先于速率限制执行；预算熔断激活后跳过速率限制直接拒绝请求
- 2FA 开启后新设备登录需验证码，丢失验证器需走恢复流程
- 设备吊销后该设备所有会话立即失效
- API Key 权限遵循最小权限原则，可限制模型/IP 范围

**常见问题**：
Q: 预算熔断后还能调用吗？
A: 熔断激活后所有请求直接返回 QUOTA_EXCEEDED，需调整预算或充值后解除。

Q: 忘记 2FA 验证器怎么办？
A: 使用备用恢复码，或联系管理员走身份验证流程重置。

Q: 登录异常提示准确吗？
A: 基于登录 IP/设备/行为分析，异常检测结果仅供参考，可查看登录历史核对。
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 开启 2FA | 绑定 TOTP 验证器开启双因素认证 |
| 关闭 2FA | 关闭双因素认证（需验证当前验证码） |
| 生成恢复码 | 生成备用恢复码（仅显示一次） |
| 重置 2FA（管理员） | 为用户重置 2FA 绑定 |

**§20.3 设备管理**

| 按钮/操作 | 帮助说明 |
|----------|---------|
| 查看设备列表 | 查看所有已登录设备（设备/时间/IP） |
| 吊销设备 | 强制登出指定设备 |
| 吊销全部 | 登出所有设备（除当前设备） |
...
| 按钮/操作 | 帮助说明 |
|----------|---------|
| 配置 Key 权限 | 限制 Key 可访问的模型/IP/操作范围 |
| 查看权限 | 查看 Key 的当前权限配置 |
| 一键收紧 | 快速将 Key 权限收紧到最小必要范围 |

**§20.5 登录异常检测**

| 按钮/操作 | 帮助说明 |
|----------|---------|
| 查看异常提示 | 查看异地/异常设备登录提示 |
| 查看登录历史 | 核对所有登录记录（时间/IP/设备） |
| 标记为本人 | 将异常登录标记为本人操作（减少误报） |
