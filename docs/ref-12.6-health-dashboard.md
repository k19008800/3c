...
| 渠道连通性 | 检查所有渠道 API 是否可达 | 所有渠道最近 1 分钟至少 1 次成功调用 |
...
| 方法 | 路径 | 状态 | 说明 | 权限 |
|------|------|------|------|------|
| `GET` | `/api/v1/admin/health/overview` | ✅ 已实现 | 服务状态总览 + 渠道通道健康统计 | security_view |
...
| `GET` | `/api/v1/admin/health/vendors` | 待实现 | 渠道通道健康详表 | security_view |
...
| 渠道通道数量 > 50 | 按健康率排序，默认展示异常/警告通道，可展开查看全部 |
...
1. 健康大屏展示所有服务状态（API/数据库/Redis/渠道/磁盘/安全）
