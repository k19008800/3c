import { Loader2, Download, Trash2, Send, X, ToggleLeft, ToggleRight } from 'lucide-react'
...
interface Props {
  ...
  onToggleSelectCode: (id: number) => void
  onBatchAction: (action: 'disable' | 'enable') => void
  onExport: () => void
  onRevoke: (id: number) => void
  onOpenGiftModal: (codeId: number, codeDisplay: string) => void
}
                  ...
                  <tr key={c.id} className={`hover:bg-slate-50 transition ${isSelected ? 'bg-purple-50' : ''}`}>
                    <td className="px-4 py-3">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={() => onToggleSelectCode(c.id)}
                        className="rounded border-slate-300"
                      />
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {c.status === 'unused' && (
                          <>
                            <button
                              onClick={() => onOpenGiftModal(c.id, c.code)}
