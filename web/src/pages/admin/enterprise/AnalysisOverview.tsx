...
import {
  Loader2, Building2, Wallet, Activity, AlertCircle,
  TrendingUp, Zap, DollarSign, TrendingDown, Search, BarChart3,
} from 'lucide-react'
import MiniChart from '@/components/ui/MiniChart'
import {
  type EnterpriseOverview, type TopConsumer, type DaySeries,
  DATE_RANGES, STATUS_OPTIONS, fmt, fmtCompact, CHART_TOOLTIP_STYLE,
} from './types'
...
/* ════════════════════════════════════════
   StatCard — 概览统计卡片（带 MiniChart）
   ════════════════════════════════════════ */
export function StatCard({
  icon, label, value, sub, color, disabled, chartData,
}: {
      ...
      {chartData && chartData.length > 1 && (
        <div className="mt-1.5" style={{ height: 32 }}>
          <MiniChart
            ...
            width={220}
            height={32}
            color={color ? color.replace('text-', '#') : '#0984e3'}
            gradient
          />
        </div>
      )}
...
interface OverviewPanelProps {
  ...
  daysRange: number
  onSelect: (user: { id: number; email: string; nickname: string | null; companyName: string | null; balance: string; lastLoginAt: string | null; status: string | null }) => void
  onDaysChange: (days: number) => void
}

export default function AnalysisOverview({
  overview, topConsumers, enterpriseTrend, daysRange, onSelect, onDaysChange,
}: OverviewPanelProps) {
  // 为概览卡片生成 MiniChart 数据
  const trendMiniData = useMemo(() => {
    if (!enterpriseTrend || enterpriseTrend.length === 0) return null
    return {
      calls: enterpriseTrend.map(d => ({ value: d.calls.total })),
      tokens: enterpriseTrend.map(d => ({ value: Math.round(d.calls.totalTokens / 10000) })),
      cost: enterpriseTrend.map(d => ({ value: parseFloat(d.calls.totalCost) })),
    }
  }, [enterpriseTrend])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={<Building2 size={15} />} label="企业总数"
          value={overview ? String(overview.totalEnterprises) : '—'}
          sub={overview ? `本月新增 ${overview.monthNewEnterprises} 家` : ''}
          color="text-blue-600"
          chartData={trendMiniData?.calls}
        />
        <StatCard
          icon={<Wallet size={15} />} label="企业总余额"
          value={overview ? `¥${fmt(overview.totalBalance)}` : '—'}
          color="text-emerald-600"
          chartData={trendMiniData?.cost}
        />
        <StatCard
          icon={<Activity size={15} />} label="月活跃企业"
          value={overview ? String(overview.activeEnterprises) : '—'}
          sub={overview && overview.totalEnterprises > 0
            ? `活跃率 ${(overview.activeEnterprises / overview.totalEnterprises * 100).toFixed(1)}%` : ''}
          color="text-violet-600"
        />
        <StatCard
          icon={<AlertCircle size={15} />} label="低余额企业"
          value={overview ? String(overview.lowBalanceEnterpriseCount) : '—'}
          sub={overview && overview.lowBalanceEnterpriseCount > 0 ? '余额 < ¥10' : ''}
          color="text-amber-600"
        />
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={<TrendingUp size={15} />} label="本月企业总消费"
          value={overview ? `¥${fmt(overview.monthConsumption.totalCost)}` : '—'}
          sub={overview ? `${fmtCompact(overview.monthConsumption.totalCalls)} 次调用` : ''}
          color="text-orange-600"
          chartData={trendMiniData?.cost}
        />
        <StatCard
          icon={<Zap size={15} />} label="本月企业 Token"
          value={overview ? `${fmtCompact(overview.monthConsumption.totalTokens)}` : '—'}
          color="text-purple-600"
          chartData={trendMiniData?.tokens}
        />
        <StatCard
          icon={<DollarSign size={15} />} label="本月企业充值"
          value={overview ? `¥${fmt(overview.monthRecharge.total)}` : '—'}
          sub={overview ? `${overview.monthRecharge.count} 笔` : ''}
          color="text-emerald-600"
        />
        <StatCard
          icon={<TrendingDown size={15} />} label="昨日消费"
          value={overview ? `¥${fmt(overview.yesterdayConsumption)}` : '—'}
          color="text-sky-600"
        />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-slate-800">企业整体趋势</h3>
          </div>
          {dateRangeButtons}
        </div>
        <div className="p-5">
          {!enterpriseTrend ? (
            ...
            <>
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={enterpriseTrend.map(d => ({
                  date: d.date.slice(5),
                  calls: d.calls.total,
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <BarChart3 size={16} className="text-slate-600" />
            <h3 className="text-sm font-semibold text-slate-800">企业消费排行 Top 10</h3>
          </div>
        </div>
              ...
              <tbody>
                {topConsumers.map((c, i) => (
                  <tr key={c.userId}
                    ...
                    onClick={() => onSelect({
                      id: c.userId, email: c.email, nickname: c.nickname,
                      companyName: c.companyName, balance: c.balance,
                      lastLoginAt: null, status: null,
                    })}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-[11px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 font-medium">企业</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {/* 低余额企业预警 */}
      {overview?.lowBalanceEnterpriseList && overview.lowBalanceEnterpriseList.length > 0 && (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {overview.lowBalanceEnterpriseList.map(u => (
                  <tr key={u.id}
                    className="border-b border-amber-50 hover:bg-amber-50/50 transition cursor-pointer"
                    onClick={() => onSelect({
                      id: u.id, email: u.email, nickname: u.nickname,
                      companyName: u.companyName, balance: u.balance,
                      lastLoginAt: u.lastLoginAt, status: 'active',
                    })}>
      ...
      <div className="text-center py-6">
        <div className="inline-flex items-center gap-2 text-sm text-slate-400 bg-slate-50 px-4 py-2 rounded-full">
          <Search size={14} />在上方搜索框输入企业名称或邮箱，查看详细分析
        </div>
      </div>
    </div>
  )
