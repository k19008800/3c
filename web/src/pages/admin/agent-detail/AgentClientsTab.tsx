// ═══════════════════════════════════════════════════
//  AgentClientsTab — 下属客户列表（集成 AgentClients.tsx）
//  含自定义 hook、客户表格、绑定弹窗
// ═══════════════════════════════════════════════════
...
function useAgentClients(agentId: number, page: number, pageSize: number) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await get<AgentClientDetail>(
        `/api/v1/admin/agents/${agentId}/clients`,
        { page, pageSize }
      )
      setData(res)
    } catch (err: any) {
...
interface ClientTableProps {
  data: AgentClientDetail | null
  loading: boolean
}

function ClientTable({ data, loading }: ClientTableProps) {
  return (
...
interface BindClientModalProps {
  agentId: number
  onClose: () => void
}

function BindClientModal({ agentId, onClose }: BindClientModalProps) {
...
export default function AgentClientsTab({ agentId }: AgentClientsTabProps) {
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(20)
  const [showBind, setShowBind] = useState(false)

  const { data, loading, error, refetch } = useAgentClients(
    agentId,
    page,
    pageSize
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        {data && (
          <p className="text-sm text-slate-500">
            ...
            {' · '}
            分佣{' '}
            {data.agent.commissionRate
              ? `${(Number(data.agent.commissionRate) * 100).toFixed(1)}%`
              : '未设置'}
            {' · '}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <ClientTable data={data} loading={loading} />
