...
export default function ModelForm({ mode, item, onClose, onSuccess }: ModelFormProps) {
  ...
  const [form, setForm] = useState(() => 
    mode === 'edit' && item ? fromItem(item) : emptyForm()
  )
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = buildPayload(form, mode === 'create')
      
      if (mode === 'create') {
        await post('/api/v1/admin/vendor-models', body)
      } else if (item) {
        await put(`/api/v1/admin/vendor-models/${item.id}`, body)
      }
      
      onSuccess()
    } catch (err: any) {
      setMessage(`${mode === 'create' ? '创建' : '更新'}失败：` + (err.message || ''))
    } finally {
      setSubmitting(false)
    }
  }
