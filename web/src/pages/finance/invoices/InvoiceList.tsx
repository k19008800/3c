...
import { statusLabel, statusColor, invoiceTypeLabel, STATUS_OPTIONS } from './types'
                ...
                {list.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-6 py-4 text-slate-600">
                      {invoiceTypeLabel[item.invoiceType] || item.invoiceType}
                    </td>
                    <td className="px-6 py-4 text-slate-700 max-w-[160px] truncate" title={item.title}>
                      {item.title}
                    </td>
